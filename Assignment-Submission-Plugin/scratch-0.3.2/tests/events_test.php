...
class assignsubmission_scratch_events_testcase extends advanced_testcase {
    ...
    /**
     * Setup all the various parts of an assignment activity including creating an scratch submission.
     */
    protected function setUp() {
        ...
        $this->data->scratch_editor = array(
    ...
    /**
     * Test that the assessable_uploaded event is fired when an Scratch submission is saved.
     */
    public function test_assessable_uploaded() {
        $this->resetAfterTest();

        $plugin = $this->assign->get_submission_plugin_by_type('scratch');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\assignsubmission_scratch\event\assessable_uploaded', $event);
    ...
    /**
     * Test that the submission_created event is fired when an scratch submission is saved.
     */
    public function test_submission_created() {
        ...
        $this->assertCount(2, $events);
        $event = $events[1];
        $this->assertInstanceOf('\assignsubmission_scratch\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    /**
     * Test that the submission_updated event is fired when an scratch
    ...
    public function test_submission_updated() {
        ...
        $event = $events[3];
        $this->assertInstanceOf('\assignsubmission_scratch\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
