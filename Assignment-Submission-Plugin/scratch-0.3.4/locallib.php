...
class assign_submission_scratch extends assign_submission_plugin {
    ...
    public function get_form_elements($submission, MoodleQuickForm $mform, stdClass $data) {
        ...
        //Print ID
        print_r($scratchid);
        $mform->addElement('html', '<h4> Please click on your project you want to submit:</h4>');
        //$mform->addElement('text', 'name', 'Please click on the project you want to submit:');

        //MENU


        //$json_result = $this->get_student_project_data_test($scratchid);
        //debugging($projjson);

        //Get images and project ids to populate the selection.
        //Returns key value pairs of ids and img urls.

        $projectsjson = $this->get_student_projects($scratchid);

        //Iterate over JSON and get titles, thumbnails, and IDs
        //Generate thumbnails
        $radioarray=array();
        foreach ($projectsjson as $index => $key){
          $p = $projectsjson[$index];
          $imagehtml = '<h5>'. $p['title'] . '</h5><img src="' . $p['image'] .  '" alt="" width="240" height="180" ><br>';
          //$mform->addElement('advcheckbox', 'ratingtime', $id, $imagehtml, array('group' => $id), array(0, 1));
          $radioarray[] = $mform->createElement('radio', 'selection', $imagehtml, '', $p['id']);
        }
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        if ($scratchsubmission) {
            ...
            //JSON
            $scratchid = $USER->profile['scratchid'];
            $externaljson = $this->get_single_project($scratchid, $data->selection);
            $internaljson = file_get_contents($scratchsubmission->projremix);

            $scratchsubmission->json = $internaljson;
    ...
    public function get_editor_format($name, $submissionid) {
     ...
     /**
      * Displays in the portfolio once submitted. (+)
      *
      * @param stdClass $submission
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        if ($scratchsubmission) {
            ...
            if ($text != $shorttext) {
                ...
                //return $scratchsubmission->metajson;
                //return $scratchsubmission->projectjson['title'];
                return $scratchsubmission->projremix;

            } else {
                //return $plagiarismlinks . $shorttext;
                debugging();
                //print_object($scratchsubmission);
                return $scratchsubmission->projremix;
                //return "whence";
            }
    ...
    /**
     * Display the saved text content from the editor in the view table. Note: text not visible until clicked on
     *
     * @param stdClass $submission
     * @return string
     */
    public function view(stdClass $submission) {
        ...
        if ($scratchsubmission) {
            ...
            //TODO: //it would be nice if you could just call external SON object to get names and image
            $result = $scratchsubmission->json;
        }
    ...
    public function get_student_project_data_old($scratchid){
    ...
    /*Gets and returns JSON for all a user's projects
    *
    */
    public function get_student_projects($scratchid){
      $r = file_get_contents('https://api.scratch.mit.edu/users/'.$scratchid . '/projects');
    ...
    /**
    * Gets and returns JSON for a single project
    */
    public function get_single_project($scratchid, $projectid){
      $r = file_get_contents('https://api.scratch.mit.edu/users/'.$scratchid . '/projects');
